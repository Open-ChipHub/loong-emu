...
void gdb_init_cpu(CPUState *cpu)
{
    ...
    feature = gdb_find_static_feature(gdbserver_state.gdb_core_xml_file);
    gdb_register_feature(cpu, 0,
                         loongarch_cpu_gdb_read_register, loongarch_cpu_gdb_write_register,
                         feature);
...
void gdb_put_buffer(const uint8_t *buf, int len)
{
    ...
    while (send_len > 0) {
restart_send:
        send_bytes = write(sock_fd, buf + send_buf_bytes, send_len);
        if (send_bytes == -1)
            goto restart_send;
        send_buf_bytes += send_bytes;
        send_len -= send_bytes;
    }
    
    // assert(send_buf_bytes == len);
    if (gdb_verbose && 0) {
        fprintf(stderr, "send %d bytes: 0x", len);
        for (int i = 0; i < len; i++) {
                        ...
                        void *ptr, size_t len, bool is_write)
{
    hwaddr phys_addr;
    int prot;
    uint8_t *array_ptr = ptr;

    CPUArchState *env = cpu_env(cpu);

    if (get_physical_address_debug(env, &phys_addr, &prot, addr, MMU_DATA_LOAD,
                                   cpu_mmu_index(cpu, false)) != 0) {
        return -1;
    }
    if (!is_write) {
        for (int i = 0; i < len; i++) {
            if (!ram_ldub_check(phys_addr + i, array_ptr + i))
                return -1;
        }
    } else {
        for (int i = 0; i < len; i++) {
            if (!ram_stub_check(phys_addr + i, array_ptr + i))
                return -1;
        }
    }
    return 0;
...
{
    int res = -1;
    // We Dummy This Instruction Access when GDB
    if (!cpu_memory_rw_debug(cpu, addr, buf, len, is_write)) {
        return 0;
    }

    uint8_t temp_buf[4] = {0x02, 0x60, 0x00, 0xef};
    uint64_t bpc;
    uint64_t new_bpc;
    for (int i = 0; i < GDB_FETCCH_BREAKPOINT_NUM; i++) {
        bpc = gdb_fetch_breakpoint[i];
        if(bpc) {
            new_bpc = (bpc < TARGET_PAGE_SIZE) ? TARGET_PAGE_SIZE : bpc;
            if ((addr < new_bpc + TARGET_PAGE_SIZE) && 
                (addr > new_bpc - TARGET_PAGE_SIZE) ) {
                for (int i = 0; i < len; i++) {
                    buf[i] = temp_buf[i%4];
                }
                return 0;
...
extern int gdb_breakpoint_check(CPUState *cs, int type, vaddr addr);

int gdb_breakpoint_insert(CPUState *cs, int type, vaddr addr, vaddr len)
{
    // check addr and jump helper function
    if (gdb_breakpoint_check(cs, type, addr))
        return 0;

    if (type == 0) {
        for (int i = 0; i < GDB_FETCCH_BREAKPOINT_NUM; i++) {
            if (gdb_fetch_breakpoint[i] == ~0x0UL) {
...
GArray *gdb_get_register_list(CPUState *cpu)
{
    ...
    for (int f = 0; f < ((GArray*)cpu->gdb_regs)->len; f++) {
        GDBRegisterState *r = &g_array_index((GArray*)cpu->gdb_regs, GDBRegisterState, f);
...
static void cpu_set_pc(CPUState *cpu, const uint64_t target){
    cpu->env->pc = target;
}
...
static void handle_v_kill(GArray *params, void *user_ctx)
{
    /* Kill the target */
    gdb_put_packet("OK");
    printf("QEMU: Terminated via GDBstub");
    exit(0);
}
...
static void handle_query_thread_extra(GArray *params, void *user_ctx)
{
    ...
    if (gdbserver_state.multiprocess && (gdbserver_state.process_num > 1)) {
        ...
        const char *cpu_model = "dump_cpu_model";
        const char *cpu_name = "dump_cpu_name";
...
static void handle_query_supported(GArray *params, void *user_ctx)
{
    ...
    if (gdbserver_state.gdb_core_xml_file) {
...
static int gdb_handle_packet(const char *line_buf)
{
    ...
    case 'k':
        /* Kill the target */
        fprintf(stderr, "QEMU: Terminated via GDBstub");
        exit(0);
        break;
    case 'D':
        {
